...
import { checkWindow, submitMCM } from "../../services/api";
...
function ScholarshipForm() {
  ...
  const form = useForm({
    ...
    initialValues: {
      award_type: "Merit-cum-means Scholarship",
      brother_name: "",
      brother_occupation: "",
      sister_name: "",
      sister_occupation: "",
  ...
  const handleNext = (e) => {
    ...
    if (!confirmed) return;
    form.validate();
    if (Object.keys(form.errors).length === 0) setStep(2);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitMCM(submissionData);
      alert("Form data and documents submitted successfully.");
    } catch (submitErr) {
      console.error("An error occurred:", submitErr);
  ...
  useEffect(() => {
    ...
    checkWindow("Merit-cum-means Scholarship")
      .then((data) => {
        if (!cancelled) {
          setEligibility({
      ...
      .catch((err) => {
        ...
        if (!cancelled) {
          setEligibility({
            ...
            message:
              (err && err.message) ||
              "Could not verify the application window. You can still try to submit.",
          });
        }
      })
      .finally(() => {
    ...
    <Container size="lg">
      <Paper radius="md" p="sm">
        <Alert
          ...
          title={windowOpen ? "Application window open" : "Application window"}
          mb="md"
        >
          {eligibility.message ||
            (windowOpen
              ? "You may submit your MCM application."
              : "The official window may be closed; the form is available below.")}
        </Alert>
        <Title order={2} mb="lg">
          {step === 1
